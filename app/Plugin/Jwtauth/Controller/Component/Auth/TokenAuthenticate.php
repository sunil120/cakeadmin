<?php
/**
 * Custom Auth Exrend Plugin.Component
 * @category API
 */
...
class TokenAuthenticate extends BaseAuthenticate
{
    ...
    public function getUser(CakeRequest $request)
    {
        if (!empty($this->settings['header'])) {
            ...
            try {
                $token = JWT::decode($token, Configure::read('Security.salt'), array('HS256'));
            } catch (Exception $exc) {
                throw new Exception(__('Invalid Request'), 500);
            }
            $username = $token->user_name;
            if ($token) {
                return $this->_findUser($username, null);
            }
        }
