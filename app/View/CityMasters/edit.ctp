...
<div class="col-lg-9">
    ...
    <div class="form-group clearfix">
        ...
        <div class="col-md-6">
            <?php
            echo $this->CustomForm->input('state_id', array(
                'options' => $states,
                'empty' => 'Select State',
               ),
               array('class' => 'select2')        
    ...
    <div class="form-group clearfix">
        ...
        <div class="col-md-6">
            <?php echo $this->CustomForm->button('Submit', array('class' => 'btn btn-primary btn-flat')); ?>
...
$("#CityMasterCountryId").on("change",function(){
    if($(this).val() != '') {
        $('#CityMasterStateId').empty().append('<option value="">Select State</option>');
        $("#select2-CityMasterStateId-container").text('Select State');
        $.post('<?php echo Router::url('/', true); ?>CityMasters/getStateByCountry', {country:$(this).val()}).success(function(data) {
            data = $.parseJSON(data);
